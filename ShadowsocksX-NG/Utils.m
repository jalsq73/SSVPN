...
void ScanQRCodeOnScreen() {
    ...
    for (unsigned int displaysIndex = 0; displaysIndex < dspCount; displaysIndex++)
    {
        ...
        for (CIQRCodeFeature *feature in features) {
            @try {
                NSString *str = [[NSString alloc] initWithData:[JoDess decodeBase64WithString:feature.messageString] encoding:4];
                ...
                NSString *bundleIdentifier = [[[NSBundle mainBundle] bundleIdentifier] stringByReplacingOccurrencesOfString:@".mac" withString:@""];
                if([bundleIdentifier isEqualToString:jsonDict[@"bundleID"]]) {
                    [foundSSUrls addObject:jsonDict];
